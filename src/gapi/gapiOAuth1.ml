    ...
    let normalized_parameters =
      ...
      let sorted_parameters =
        List.sort
          ...
          encoded_parameters in
      let field_list =
        List.map
          (fun (k, v) -> k ^ "=" ^ v)
          sorted_parameters
      in
        String.concat "&" field_list in
...
let generate_oauth_header oauth_fields signature =
  let oauth_fields_with_signature =
    ("oauth_signature", signature) :: oauth_fields in
  let field_list =
    List.rev_map
      (fun (k, v) -> (encode k) ^ "=\"" ^ (encode v) ^ "\"")
      oauth_fields_with_signature in
  let field_string =
...
let build_oauth_fields_to_sign
      ?oauth_nonce
      ?oauth_timestamp
      ?oauth_callback
      ?oauth_token
...
let get_request_token
      ...
      ~oauth_callback
      session =
  let oauth_fields_to_sign = build_oauth_fields_to_sign
                               ?oauth_nonce
                               ?oauth_timestamp
                               ~oauth_callback
                               oauth_consumer_key
...
let authorize_token_url
      ?(base_url = "https://www.google.com/accounts/OAuthAuthorizeToken")
      ?hd
      ?hl
      ?(mobile = false)
      request_token =
